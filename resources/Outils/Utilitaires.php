...
abstract class Utilitaires {
   ...
   public static function ajouterMessage($msg): void {
    if (!isset($_REQUEST['success'])) { // Correction de la faute de frappe ici
    ...
    public static function donnerMoisSuivant(string $date): string
    {
        $format = "Ym";
        $dateObj = \DateTime::createFromFormat($format, $date);
        $dateObj->modify('+1 month');
        $nouvelleDate = $dateObj->format($format);
